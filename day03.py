...
day3_input = """.........874.772...........787..........556.....292......141................910............54...............................................
    ...
    for y in range(grid_size):
        ...
        for x in range(grid_size):
            if grid[y][x].isdigit():
                ...
                if current_num_x == -1:
                    current_num_x = x
            if (not grid[y][x].isdigit() or x == grid_size - 1) and current_num != "":
                number = {
...
def is_valid_coord(x, y, grid_size):
...
def is_part_number(n, grid):
    grid_size = len(grid)

    # Check for symbols above and below
    for x in range(n["x"] - 1, n["x"] + n["length"] + 1):
        for y in [n["y"] - 1, n["y"] + 1]:
            if is_valid_coord(x, y, grid_size) and is_symbol(grid[y][x]):
                return True

    # Check for symbols left and right
    for x in [n["x"] - 1, n["x"] + n["length"]]:
        y = n["y"]
        if is_valid_coord(x, y, grid_size) and is_symbol(grid[y][x]):
            return True
...
def is_adjacent(asterisk, number):
    y = number["y"]
    for x in range(number["x"], number["x"] + number["length"]):
        if abs(x - asterisk["x"]) <= 1 and abs(y - asterisk["y"]) <= 1:
            return True
    return False


def find_gear_part_pairs(asterisks, numbers):
    gear_part_pairs = []
    for asterisk in asterisks:
        adjacent_numbers = [n for n in numbers if is_adjacent(asterisk, n)]
        if len(adjacent_numbers) == 2:
            gear_part_pairs.append(tuple(adjacent_numbers))
    return gear_part_pairs


def part_1(text):
    grid = text.splitlines()
    numbers = find_numbers(grid)
    part_numbers = [n for n in numbers if is_part_number(n, grid)]
...
def part_2(text):
    grid = text.splitlines()
    numbers = find_numbers(grid)
    asterisks = find_asterisks(grid)
    gear_part_pairs = find_gear_part_pairs(asterisks, numbers)
    gear_ratios = [math.prod(n["value"] for n in p) for p in gear_part_pairs]
    return sum(gear_ratios)


if __name__ == '__main__':
    print("Part 1: %d" % (part_1(day3_input)))
    print("Part 2: %d" % (part_2(day3_input)))

...
def find_last_digit(s):
    return find_first_digit(reversed(s))


def find_first_and_last_digits(s):
    first_index = 999999
    last_index = -1
    first = 0
    last = 0
    for key in text_to_digit_map.keys():
        first_index_of_key = s.find(key)
        if -1 < first_index_of_key < first_index:
            first = text_to_digit_map[key]
            first_index = first_index_of_key
        last_index_of_key = s.rfind(key)
        if -1 < last_index_of_key > last_index:
            last = text_to_digit_map[key]
            last_index = last_index_of_key
    return first, last
...
def part_1(text):
    ...
    for line in text.splitlines():
        total += find_last_digit(line) + (10 * find_first_digit(line))
    return total
...
def part_2(text):
    total = 0
    for line in text.splitlines():
        first_digit, last_digit = find_first_and_last_digits(line)
        total += last_digit + (10 * first_digit)
    return total


if __name__ == '__main__':
    print("Part 1: %d" % (part_1(day1_input)))
    print("Part 2: %d" % (part_2(day1_input)))

...
input_text = """.F-7F-..|-F-|F---7J7FLF-7F|FJ-LJ.7--F7-|77.|7J.FFLF7-FJ7.J-7-|.J.7.F7L7..FF.|F--F--|J-FFJFL7F7-LJFF-.7F|7.FF|7FF7FL7L--F--77-7F7.7.-F-J.JFLF
...
def choose_first_pos(grid, start):
    row, col = start
    for pos in [
        (row + 1, col),
        (row - 1, col),
        ...
        candidate = grid[pos[0]][pos[1]]
        if start in get_options(candidate, (pos[0], pos[1])):
            return pos


def next_position(val, curr, prev=None):
    opts = get_options(val, curr)
    return choose_direction(opts[0], opts[1], prev)


def choose_direction(opt1, opt2, prev):
    if prev is None or prev == opt2:
        return opt1
    else:
        return opt2


def get_options(val, position):
    row, col = position
    if val == "|":
...
def part_1(text):
    ...
    curr = choose_first_pos(grid, start)
    prev = start
    count = 1
    while curr != start:
        val = grid[curr[0]][curr[1]]
        temp = curr
        curr = next_position(val, curr, prev)
        prev = temp
        count += 1
    return count / 2

...
day2_input = """Game 1: 7 blue, 5 red; 10 red, 7 blue; 5 blue, 4 green, 15 red; 4 green, 6 red, 7 blue; 5 green, 8 blue, 4 red; 5 red, 4 blue, 3 green
...
def parse_games(text):
    ...
    for line in text.splitlines():
        game_text = line.split(":")[0]
        game_num = int(game_text.split(" ")[1])
        cube_text = line.split(":")[1]
        cube_sets = []
        for cube_set_text in cube_text.split(";"):
            cube_color_counts = {}
            for cube_color_text in cube_set_text.split(","):
                count_color = cube_color_text.strip().split(" ")
                cube_color_counts[count_color[1]] = int(count_color[0])
            cube_sets.append(cube_color_counts)
...
def find_min_colors(game):
    ...
    for game_round in game:
        for color in ["red", "green", "blue"]:
            if game_round.get(color, 0) > min_colors[color]:
                min_colors[color] = game_round[color]
    return min_colors


def is_round_possible(game_round, constraints):
    for color in ["red", "green", "blue"]:
        if game_round.get(color, 0) > constraints[color]:
            return False
    return True
...
def is_game_possible(game, constraints):
    for game_round in game:
        if not is_round_possible(game_round, constraints):
            return False
    return True
...
def part_1(text):
    ...
    for game_id in games.keys():
        if is_game_possible(games[game_id], {"red": 12, "green": 13, "blue": 14}):
            sum_of_ids += game_id
    return sum_of_ids
...
def part_2(text):
    ...
    sum_of_powers = 0
    for game_id in games.keys():
        min_colors = find_min_colors(games[game_id])
        power = math.prod(min_colors.values())
        sum_of_powers += power
    return sum_of_powers


if __name__ == '__main__':
    print("Part 1: %d" % (part_1(day2_input)))
    print("Part 2: %d" % (part_2(day2_input)))
